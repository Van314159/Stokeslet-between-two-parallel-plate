...
import mpmath as mp
import numpy as np
from mpmath import sinh, cosh, sqrt, sin
import time as time
import os as os
...
#1
def chshMinshch(m, x3 ,h, H):
    return x3/H*cosh(x3*zm(m)/H)*sinh(h*zm(m)/H)-h/H*sinh(x3*zm(m)/H)*cosh(h*zm(m)/H)
...
#bb = sinh((x3-h)*zm(n)/H) + zm(n)*cosh((x3+h)*zm(n)/H) - sqrt(1+zm(n)**2)*sinh((x3+h)*zm(n)/H)

#r1 = x ,  r2 = y
def u31(r, x3, h, H, n, r1):
    return -(r1/r)*(pi/H)*mp.im( ( zm(n) * mp.hankel1(1,r*zm(n)/H) * inverseOfzm_Min(n) )* (    
  (h*x3*zm(n)/(H**2))*(sinh((x3-h)*zm(n)/H) + zm(n)*cosh((x3+h)*zm(n)/H) - sqrt(1+zm(n)**2)*sinh((x3+h)*zm(n)/H) )  + zm(n) * ( chshMinshch(n, x3 ,h, H) )   + zm(n)*(shsh(n, x3, h, H))*( (h-x3)/H*sqrt(1+zm(n)**2)+((x3+h)/H-1) )))
        

def u32(r, x3, h, H, n, r2):
    return -(r2/r)*(pi/H)*mp.im( ( zm(n) * mp.hankel1(1,r*zm(n)/H) * inverseOfzm_Min(n) )* (    
  (h*x3*zm(n)/(H**2))*(sinh((x3-h)*zm(n)/H) + zm(n)*cosh((x3+h)*zm(n)/H) - sqrt(1+zm(n)**2)*sinh((x3+h)*zm(n)/H) )  + zm(n) * ( chshMinshch(n, x3 ,h, H) )   + zm(n)*(shsh(n, x3, h, H))*( (h-x3)/H*sqrt(1+zm(n)**2)+((x3+h)/H-1) )))
    
    
def u13(r, x3, h, H, n, r1):
    return -(r1/r)*(pi/H)*mp.im( ( zm(n) * mp.hankel1(1,r*zm(n)/H) * inverseOfzm_Min(n) )* (    
  (h*x3*zm(n)/(H**2))*(sinh((x3-h)*zm(n)/H) - zm(n)*cosh((x3+h)*zm(n)/H) + sqrt(1+zm(n)**2)*sinh((x3+h)*zm(n)/H) )  + zm(n) * ( chshMinshch(n, x3 ,h, H) )   + zm(n)*(shsh(n, x3, h, H))*( (h-x3)/H*sqrt(1+zm(n)**2) - ((x3+h)/H-1) )))



def u23(r, x3, h, H, n, r2):
    return -(r2/r)*(pi/H)*mp.im( ( zm(n) * mp.hankel1(1,r*zm(n)/H) * inverseOfzm_Min(n) )* (    
  (h*x3*zm(n)/(H**2))*(sinh((x3-h)*zm(n)/H) - zm(n)*cosh((x3+h)*zm(n)/H) + sqrt(1+zm(n)**2)*sinh((x3+h)*zm(n)/H) )  + zm(n) * ( chshMinshch(n, x3 ,h, H) )   + zm(n)*(shsh(n, x3, h, H))*( (h-x3)/H*sqrt(1+zm(n)**2) - ((x3+h)/H-1) )))
